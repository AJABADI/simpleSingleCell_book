...
Ideally, we would remove cells with low library sizes or total number of expressed features as described in the `r Biocpkg("simpleSingleCell")` vignette.
...
We recommend performing the analysis first with a stringent QC filter, and then relaxing it based on further diagnostics (see `r Biocpkg("simpleSingleCell")` vignette for an example).
...
(see comments in `r Biocpkg("simpleSingleCell")` vignette for a discussion of the considerations involved in pre-clustering for normalization).
...
(see `r Biocpkg("simpleSingleCell")` vignette for details).
...
(see the vignette for the example of the @zeisel2015brain data set).
